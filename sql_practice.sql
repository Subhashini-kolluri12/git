...
foreign key(gardecode,gradelevel) references garde(gardecode,gradelevel)
);
...
gradelevel varchar(30), 
basicpay int,
primary key(empcode,changedate,desigcode,gradecode,gradelevel,basic),
...
-- List all the departments and the budget
...
select empname,desigcode from emp
where desigcode != 'SPRV';

DELETE FROM your_table_name WHERE group_id = your_group_id;

-- List the employee(s) who is senior most in the office
SELECT empname, MAX(timestampdiff(year, joindate, curdate())) as experience
FROM emp
GROUP BY empname
LIMIT 0, 1000;


/*list the employees who are working directly under superior most employee of the
...
select empname,deptname,desigcode from emp,dept
where emp.deptcode = dept.deptcode and desigcode in ('MNGR');

-- List the employees who work as ‘manager’ to at least one department
select empname,deptcode from emp
where desigcode='MNGR';

/*List the number of employees working for either ‘accounts’ or ‘personal’ or
‘purchase’,departments' */
select empname,deptcode from emp where deptcode='ACCT' or deptcode='PERS' or deptcode='PRCH';

-- List the employees working for ‘accounts’ or ‘personal’ department
select empname,deptcode from emp where deptcode='ACCT' or deptcode='PERS';

-- list the employees working for ‘accounts’ and ‘personal’ department
select empname,deptcode from emp where deptcode='ACCT' AND deptcode='PERS';
...
select emp.empname,history.basicpay from emp
inner join history on 
history.basicpay between 1000 and 1500;
...
select empname,basicpay,round(basicpay/2,0) as halfofsalary from emp;
